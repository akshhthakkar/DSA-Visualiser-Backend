...
function validateProblem(problem: unknown, index: number): string[] {
    ...
    if (p.starterCode !== undefined && (typeof p.starterCode !== 'object' || Array.isArray(p.starterCode)))
    ...
    app.post(
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            ...
            if (rawProblems.length === 0)
                throw new ValidationError('No problems provided');
            if (rawProblems.length > 50)
                throw new ValidationError(`Maximum 50 problems allowed. Your batch has ${rawProblems.length}`);
            ...
            await prisma.$transaction(async (tx) => {
                const data = newProblems.map(p => {
                    const diff = (p.difficulty.toUpperCase()) as 'EASY' | 'MEDIUM' | 'HARD';
                    const slug = `${p.title.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${Math.random().toString(36).substring(2, 7)}`;

                    const descText = typeof p.description === 'object'
                    ...
                    return {
                        ...
                        starterCode: (p.starterCode && Object.keys(p.starterCode).length) ? (p.starterCode as Prisma.InputJsonValue) : Prisma.JsonNull,
                        expectedOutput: (p.expectedOutput && Object.keys(p.expectedOutput).length) ? (p.expectedOutput as Prisma.InputJsonValue) : Prisma.JsonNull,
